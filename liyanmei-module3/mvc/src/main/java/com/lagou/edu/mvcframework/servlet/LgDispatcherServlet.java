...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private Map<Handler, Set<String>> handlerAuth = new HashMap<>();//该Handler可被那些用户访问
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            Set<String> classAuth = Collections.emptySet(); // 类级别权限: 该类下的所有方法都具有这些公共权限
            if(aClass.isAnnotationPresent(Security.class)) {
                Security annotation = aClass.getAnnotation(Security.class);
                String[] value = annotation.value();
                classAuth = new HashSet(Arrays.asList(value));
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                Set<String> methodAuth = new HashSet<>(); // 方法级别权限：公共权限 + 特殊权限
                if(method.isAnnotationPresent(Security.class)){
                    Security security = method.getAnnotation(Security.class);
                    String[] value = security.value();
                    methodAuth = new HashSet(Arrays.asList(value)); //该方法具有的特殊权限
                }

                if(!classAuth.isEmpty()){
                    methodAuth.addAll(classAuth);
                }
                System.out.println(method.getName() + " = " + methodAuth);
                ...
                handlerAuth.put(handler, methodAuth);
...
//        method.invoke() //
        ...
        // 判断用户是否有访问该 Handler 的权限
        String username = req.getParameter("username");
        Set<String> auth = Collections.EMPTY_SET;
        if(!StringUtils.isBlank(username)){
            auth = handlerAuth.get(handler);
        }

        if (!auth.contains(username)) {
            resp.setHeader("Content-type", "text/html;charset=UTF-8");
            resp.setCharacterEncoding("UTF-8");
            resp.getWriter().write(username + " 不能访问 " + handler.getMethod().getName());
            return;
        }
        // 参数绑定
